...
int read_input( char *buf , unsigned int size ){
    int ret = __read_chk( 0 , buf , size , size ); // No terminated NULL
    if(ret <= 0){
        puts("read error");
        _exit(1);
    }
...
void add(){
    for( int i = 0 ; i < MAX ; ++i ){
        if( !notes[i].data || notes[i].is_freed ){
            ...
            printf( "Note: " );
            read_input( notes[i].data , size - 1 ); // size 0 bug, lead to heap overflow!

            printf( "Description of this note: " );
            
            // fixed overflow
            // scanf( "%s" , notes[i].description ) // overflow
            scanf( "%48s" , notes[i].description ); // safe // Terminated Null will overflow!
